...
if uploaded_file is not None:
    # 1. Immediate OpenCV Auto-Crop
    with st.spinner("Isolating Document from Background..."):
        cropped_img = scan_document(Image.open(uploaded_file))
        
    st.image(cropped_img, caption="Isolated Document Preview", width=400)
    ...
    if extract_btn:
        def get_ocr_text(img_t, prefix=""):
            ...
            for attempt in range(max_r + 1):
                ...
                with st.spinner(f"{prefix}Reading OCR..."):
                    ...
                    data = {"apikey": OCR_SPACE_API_KEY, "language": "eng", "isTable": True, "scale": True}
                    try:
        ...
        with col_r:
            text_raw = get_ocr_text(Image.open(uploaded_file), "[Raw Photo] ")
...
- product_name: The descriptive name of the item (e.g., 'STORWEL ACE' or 'WARDROBE')
...
Example Context 2:
Text: "MARKETED & MANUFACTURED BY PRODUCT CODE 30161803SD01708 PRODUCT WARDROBE STORWEL ACE NET QUANTITY 1 UNIT MRP 27376 (INCL) USP 27376.00 PACKAGE 1 OF 1"
Output: {{"product_code": "30161803SD01708", "product_name": "WARDROBE STORWEL ACE", "quantity": 1, "mrp": 27376, "package": "1 OF 1"}}
